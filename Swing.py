...
from common import VXLANGPE, BASEHEADER, CONTEXTHEADER, ETHHEADER, TRACEREQHEADER, NSH_NEXT_PROTO_IPV4, \
    NSH_OAM_TRACE_DEST_IP_REPORT_OFFSET, NSH_OAM_TRACE_DEST_IP_REPORT_LEN
from common import PAYLOAD_START_INDEX_NSH_TYPE1, NSH_NEXT_PROTO_ETH
from common import PAYLOAD_START_INDEX_NSH_TYPE3, IPV4_HEADER_LEN_BYTES
import decode as nsh_decode
from encode import add_sf_to_trace_pkt
from service_index import process_service_index
...
def find_service(service_type):
    ...
    """
    if service_type == SwingXOR:
        return MySwingXORService
    elif service_type == SwingOR:
        return MySwingORService
    elif service_type == SwingAND:
        return MySwingANDService
...
class BasicService(object):
    ...
    def process_trace_pkt(self, rw_data, data):
        ...
        if ipv6_addr.ipv4_mapped:
            ...
            logger.info("IPv4 destiSwingORion:port address for trace reply: %s", trace_dest_addr)
            ...
            logger.info("IPv6 destiSwingORion address for trace reply: %s", trace_dest_addr)
            self.transport.sendto(rw_data, trace_dest_addr)
...
class MySwingXORService(BasicService):
    def __init__(self, loop):
        super(MySwingXORService, self).__init__(loop)

        self.service_type = SwingXOR


class MySwingORService(BasicService):
    def __init__(self, loop):
        super(MySwingORService, self).__init__(loop)

        self.service_type = SwingOR


class MySwingANDService(BasicService):
    def __init__(self, loop):
        super(MySwingANDService, self).__init__(loop)

        self.service_type = SwingAND
...
class MySffServer(BasicService):
    ...
    def _get_packet_bearing(packet):
        """
        Parse a packet to get source and destiSwingORion info
    ...
    def _process_incoming_packet(self, data, addr):
        ...
        if nsh_decode.is_data_message(data, offset):
            ...
            # send packet to its original destiSwingORion
                ...
                if inner_packet:
                    ...
                    # Reinaldo note:
                    # UnfortuSwingORely it has to be this way. Python has poor raw socket support in
